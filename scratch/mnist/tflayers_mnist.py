...
class Net():
	...
	def __init__(self):
		...
		self.layer = tf.layers.conv2d(self.x_reshaped, 16, 3, padding='same', activation=tf.nn.relu)
		self.layer = tf.layers.max_pooling2d(self.layer, pool_size=2, strides=2)

		self.layer = tf.layers.conv2d(self.layer, 32, 3, padding='same', activation=tf.nn.relu)
		self.layer = tf.layers.max_pooling2d(self.layer, pool_size=2, strides=2)

		self.layer = tf.layers.conv2d(self.layer, 64, 3, padding='same', activation=tf.nn.relu)
		self.layer = tf.layers.max_pooling2d(self.layer, pool_size=2, strides=2, padding='same')

		print(self.layer.get_shape().as_list())

		self.layer = tf.reshape(self.layer, [-1, 4*4*64])

		self.layer = tf.layers.dense(self.layer, 500, activation=tf.nn.relu)
		self.layer = tf.layers.dense(self.layer, 10)
